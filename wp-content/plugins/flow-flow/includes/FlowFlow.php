...
require_once(AS_PLUGIN_DIR . 'includes/social/FFVine.php');
...
require_once(AS_PLUGIN_DIR . 'includes/social/FFDribbble.php');
...
require_once(AS_PLUGIN_DIR . 'includes/social/FFFoursquare.php');
...
require_once(AS_PLUGIN_DIR . 'includes/settings/FFSnapshotManager.php');
...
class FlowFlow {
	...
	private function prepareResult($all, $errors, $hash) {
		$page = (int)$_REQUEST['page'];
		$next = $page;
		$oldHash = isset($_REQUEST['hash']) ? $_REQUEST['hash'] : $hash;
		$result = array();
		if ($oldHash == $hash) {
			$all = $this->filterByDays( $all );
			$allCount = sizeof( $all );
			if ( $allCount > 0 ) {
				$count = (int) $this->settings->getCountOfPostsOnPage();
				$count = $count > $allCount ? $allCount : $count;
				if ( ! $this->settings->isRandomOrder() ) {
					usort( $all, array( $this, $this->settings->order() ) );
				}
				$result = array_slice( $all, $count * $page, $count );
				if ( ( $oldHash == $hash ) && ( $allCount > ( $count * $page + $count ) ) ) {
					$next = $next + 1;
				}
				if ( $this->settings->isRandomOrder() ) {
					usort( $result, array( $this, $this->settings->order() ) );
				}
		...
		$response = array('id' => (int)$this->settings->getId(), 'items' => $result, 'errors' => $errors, 'hash' => $oldHash, 'page' => $page, 'next_page' => $next);
	 ...
	 * @return array
	 */
	private function filterByDays($posts){
		if (false !== ($days = $this->settings->getDays()) && sizeof($posts) > 0){
			$tmp = array();
	...
	private static function single_activate() {
		FFSnapshotManager::create_snapshot_table();
	}
...
//		wp_localize_script(self::$PLUGIN_SLUG . '-plugin-script', 'FlowFlowOpts', $js_opts);
	...
	public function register_settings() {
		...
		add_settings_field('foursquare_client_id', '', '', $option_name, '');
		add_settings_field('foursquare_client_secret', '', '', $option_name, '');
	}
	...
	public function validate_options($plugin_options) {
		...
		//$plugin_options['oauth_access_token'] = trim($plugin_options['oauth_access_token']);
