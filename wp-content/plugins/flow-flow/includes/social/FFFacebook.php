    ...
    protected function showImage($item){
	    ...
	    if (($item->type == 'video')
		    ...
		    if (isset($item->object_id) && trim($item->object_id) != ''){
			    ...
			    $data = FFFeedUtils::getFeedData("https://graph.facebook.com/{$item->object_id}?fields=embed_html");
			    ...
			    if ((strpos($item->source, 'youtube.com') > 0) || (strpos($item->source, 'vimeo.com') > 0)) {
				    $this->image = $this->createImage($item->picture);
...
//        }
    ...
    private function filter( $item ) {
	    if ($item->type == 'status' && ($this->hideStatus || !isset($item->message))) return;
