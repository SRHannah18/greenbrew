...
if($deviceType != 'phone') { ?>	
	...
	<div class="sevencol first left-column">
		...
		<section itemprop="articleBody">
			...
			<h2>Tasting Notes</h2>
			<blockquote>
			<p><?php echo get_post_meta( get_the_ID(), '_gfb_tasting_notes', true ); ?></p>
			</blockquote>
			<?php } ?>
...
?>
	...
	<?php } else { ?>
		...
		<section itemprop="articleBody">
			...
			<header class="beer-header">
			<h1 class="single-beer-title mobile-border-bottom clearboth" itemprop="headline"><?php the_title(); ?>
			<?php if($term->term_id == 4) { 
				$fog_number = get_post_meta( get_the_ID(), '_gfb_fog_number', true );
			?>
			<span class="flash-of-genius-txt"><span class="title">Flash of Genius</span> <span class="image-replacement number number-<?php echo $fog_number; ?>"><?php echo $fog_number; ?></span></span>
			<?php } ?>
			</h1>
