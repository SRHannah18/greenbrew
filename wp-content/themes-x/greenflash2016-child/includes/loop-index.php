...
if (have_posts()) :
	...
	$current_post['content'] 	= $blog_content == "content" ? get_the_content(__('Read now','avia_framework')) : get_the_excerpt();
	$current_post['content'] 	= $blog_content == "excerpt_read_more" ? $current_post['content'].'<div class="read-more-link"><a href="'.get_permalink().'" class="more-link btn btn-large">'.__('Read now','avia_framework').'</a></div>' : $current_post['content'];
	...
	echo "<article class='".implode(" ", get_post_class('post-entry post-entry-type-'.$post_format . " " . $post_class . " ".$with_slider))."' ".avia_markup_helper(array('context' => 'entry','echo'=>false)).">";
        ...
        echo "</div>";
*/
        echo "<div class='post-outer-wrap clearfix {$post_format}-content'>";
            echo '<header>';
