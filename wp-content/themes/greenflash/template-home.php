	...
	<div id="inner-content" class="wrap clearfix">
		...
		<div class="outerwrap twelvecol first bg-gray boxes">
			...
			if ( function_exists( 'ot_get_option' ) ) {
				...
				// this way more than one can be saved and used at a later time.
				$home_boxes = array_splice($home_boxes,0,3);

				$home_featured_beer = ot_get_option( 'home_featured_beer', array());
				$home_featured_beer = array_splice($home_featured_beer,0,1);
				$x = 1;

				foreach($home_boxes as $info) {
					if($x == 1) {$box_class = ' first';}
					elseif($x == 2) {$box_class = '';}
					else {$box_class = ' last';}
					?>
					<div class="fourcol<?php echo $box_class; ?>">
						<a href="<?php echo $info['link']; ?>"><img src="<?php echo $info['image']; ?>" alt="<?php echo $info['title']; ?>" width="327" height="327" /></a>
						<div class="box-content">
						<h2><a href="<?php echo $info['link']; ?>"><?php echo $info['title']; ?></a></h2>
						<p><?php echo $info['description']; ?> <!-- <a href="<?php echo $info['link']; ?>">more&hellip;</a> --></p>
						</div>
					<?php 
					if($x == 1) { 
						foreach($home_featured_beer as $beer) {
						?>
						<img src="<?php echo $beer['image']; ?>" alt="<?php echo $beer['description']; ?>" width="116" height="402" id="featured-beer-bottle" />
					<?php 
						} //eof beer loop
					} // eof x check
					?>
